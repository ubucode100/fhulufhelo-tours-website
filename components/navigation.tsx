...
export default function Navigation() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'bg-background text-foreground backdrop-blur-sm shadow-lg' : 'bg-transparent'
    }`}>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                href={link.href}
                className=" hover:text-white px-4  text-sm font-medium transition-colors duration-200 relative group"
              >
                {link.name}
